...
export function registerTaskInfoCommand(task: Command): void {
  task
    ...
    .action(async (spec?: string, taskId?: string) => {
      ...
      const tasksDir = path.join(workspacePath, ".codeforge", "tasks", specName);
      ...
      try {
        ...
        console.log(`DEPENDENCIES: ${taskData.dependencies && taskData.dependencies.length > 0 ? taskData.dependencies.join(", ") : "None"}`);
        console.log(`FILES TO MODIFY/CREATE: ${taskData.files && taskData.files.length > 0 ? "\n  - " + taskData.files.join("\n  - ") : "None"}\n`);
        ...
        if (taskData.acceptanceCriteria && taskData.acceptanceCriteria.length > 0) {
            ...
            taskData.acceptanceCriteria.forEach((c: string) => console.log(`- ${c}`));
            console.log();
        }
